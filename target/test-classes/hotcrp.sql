-- https://github.com/kohler/hotcrp/blob/master/src/schema.sql, modified
...
CREATE TABLE `ContactInfo` (
  ...
  `unaccentedName` varchar(240) NOT NULL DEFAULT '',
  ...
  `collaborators` varchar(8192) DEFAULT NULL,
  ...
  `contactTags` varchar(4096) DEFAULT NULL,
  `data` varchar(32767) DEFAULT NULL,
  PRIMARY KEY (`contactId`),
  UNIQUE KEY `email` (`email`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE INDEX `ContactInfo_roles`
ON `ContactInfo`(`roles`);
...
CREATE TABLE `DeletedContactInfo` (
  ...
  `unaccentedName` varchar(240) NOT NULL,
  `email` varchar(120) NOT NULL,
  `affiliation` varchar(2048) NOT NULL DEFAULT ''
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `Paper` (
  ...
  `authorInformation` varchar(8192) DEFAULT NULL,
  ...
  `withdrawReason` varchar(1024) DEFAULT NULL,
...
CREATE INDEX `Paper_timeSubmitted`
ON `Paper`(`timeSubmitted`);

CREATE INDEX `Paper_leadContactId`
ON `Paper`(`leadContactId`);

CREATE INDEX `Paper_shepherdContactId`
ON `Paper`(`shepherdContactId`);
...
CREATE TABLE `PaperComment` (
  ...
  `commentTags` varchar(1024) DEFAULT NULL,
...
CREATE INDEX `PaperComment_contactId`
ON `PaperComment`(`contactId`);

CREATE INDEX `PaperComment_timeModifiedContact`
ON `PaperComment`(`timeModified`, `contactId`);
...
CREATE INDEX `PaperConflict_paperId`
ON `PaperConflict`(`paperId`);
...
CREATE TABLE `PaperReview` (
  ...
  `tfields` longblob,
  `sfields` varchar(2048) DEFAULT NULL,
  `data` varchar(8192) DEFAULT NULL,
...
CREATE INDEX `PaperReview_contactId`
ON `PaperReview` (`contactId`);

CREATE INDEX `PaperReview_reviewType`
ON `PaperReview` (`reviewType`);

CREATE INDEX `PaperReview_reviewRound`
ON `PaperReview` (`reviewRound`);

CREATE INDEX `PaperReview_requestedBy`
ON `PaperReview` (`requestedBy`);
...
CREATE TABLE `PaperReviewRefused` (
  ...
  `data` varchar(8192) DEFAULT NULL,
  `reason` varchar(32767) DEFAULT NULL,
...
CREATE TABLE `PaperStorage` (
  ...
  `infoJson` varchar(32768) DEFAULT NULL,
...
CREATE TABLE `PaperTagAnno` (
  ...
  `tagIndex` float NOT NULL DEFAULT '0',
  `heading` varchar(8192) DEFAULT NULL,
...
CREATE TABLE `TopicArea` (
  ...
  `topicName` varchar(1024) DEFAULT NULL,
  PRIMARY KEY (`topicId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
